...
async function detectIntent(message: string): Promise<string> {
  const prompt = `
    ...
    Message: ${message}
  `
  return aiBrain('You are an intent classifier.', prompt)
}

function salesAI(message: string): Promise<string> {
  return aiBrain(
...
function supportAI(message: string): Promise<string> {
  return aiBrain('You are a helpful customer support agent.', message)
}

function gmailAI(emailText: string): Promise<string> {
  return aiBrain(
